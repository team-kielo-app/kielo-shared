...
func TestContextualizeStorageURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "loopback request rewrites to caller host (HOST_IP ignored)",
			requestHostname: "localhost",
			rawURL:          "http://gcs-emulator:4443/storage/v1/b/bucket/o/obj",
		...
		{
			name:            "loopback 127.0.0.1 rewrites to caller host",
			requestHostname: "127.0.0.1",
			rawURL:          "http://gcs-emulator:4443/storage/v1/b/bucket/o/obj",
			envHostIP:       testAltPublicHost,
			envEmulatorHost: "gcs-emulator:4443",
			want:            "http://127.0.0.1:4443/storage/v1/b/bucket/o/obj",
		},
		{
		...
		{
			name:            "FQDN caller rewrites to its own host",
			requestHostname: "api.kielo.app",
			rawURL:          "http://gcs-emulator:4443/storage/v1/b/bucket/o/obj",
			envHostIP:       testPublicHost,
			envEmulatorHost: "gcs-emulator:4443",
			want:            "http://api.kielo.app:4443/storage/v1/b/bucket/o/obj",
		},
		{
			name:            "LAN IP caller rewrites to its own host (self-corrects on network change)",
		...
		{
			name:            "loopback with port rewrites to caller host",
...
func TestContextualizeServiceURL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envHostIP:       testPublicHost,
			envEmulatorHost: "gcs-emulator:4443",
			want:            "http://localhost:4443/storage/v1/b/bucket/o/obj?alt=media",
		},
		{
			name:            "internal docker hostname rewritten to caller host for loopback request",
			requestHostname: "localhost",
		...
		{
			name:            "internal docker hostname rewrites to caller host regardless of HOST_IP",
			requestHostname: "127.0.0.1",
			rawURL:          "http://kielo-ktv-api:8080/api/v1/outputs/video.mp4",
			envHostIP:       "",
			want:            "http://127.0.0.1:8080/api/v1/outputs/video.mp4",
		},
		{
			...
			envHostIP:       testAltPublicHost,
			want:            "http://localhost:9999/some/path",
		},
		{
			...
			envHostIP:       testPublicHost,
			want:            "http://localhost:8080/api/v1/outputs/video.mp4?token=abc",
		},
		{
		...
		{
			name:            "FQDN caller rewrites internal URL to its own host",
			requestHostname: "api.kielo.app",
			rawURL:          "http://kielo-ktv-api:8080/api/v1/outputs/video.mp4",
			envHostIP:       testPublicHost,
			want:            "http://api.kielo.app:8080/api/v1/outputs/video.mp4",
		},
		{
			name:            "LAN IP caller rewrites internal URL to its own host",
			requestHostname: "192.168.1.108:8084",
			rawURL:          "http://kielo-ktv-api:8080/api/v1/outputs/video.mp4",
			envHostIP:       testPublicHost,
			want:            "http://192.168.1.108:8080/api/v1/outputs/video.mp4",
		},
		{
			name:            "loopback with port rewrites to caller host",
